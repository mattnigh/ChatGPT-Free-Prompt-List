...
function fixYamlContent(content) {
  ...
  // Get the content part (everything after the front matter)
  const contentPart = cleanParts.length > 1 ? cleanParts.slice(1).join('\n\n') : '';

  // Format the YAML front matter
  const frontMatter = yaml.dump(yamlData, {
    lineWidth: -1,
    noRefs: true,
    quotingType: '"'
  });
